"""Metadata for drf-firebase-auth-custom package."""
__title__ = 'drf-firebase-auth_custom'
__version__ = '1.0.0'
__description__ = (
    'Custom Django Rest Framework authentication backend for '
    'parsing Firebase uid tokens and storing as local users.'
)
__url__ = 'https://github.com/zuxriddin99/drf-firebase-auth_custom'
__author__ = 'original author Gary Burgmann(this is custom version)'
__author_email__ = 'original author garyburgmann@gmail.com(custom version zuz.828@gmail.com)'
__license__ = 'MIT'
__copyright__ = 'Copyright 2021 Gary Burgmann'

# Version synonym
VERSION = __version__
